...
class TEDS(object):
    ...
    def batch_evaluate(self, results_json):
        ...
        if self.n_jobs == 1:
            scores = [
                self.evaluate(
                    results_json[filename]["structure_pred"],
                    results_json[filename]["structure_gt"],
                )
            ...
            inputs = [
                {
                    "pred": results_json[filename]["structure_pred"],
                    "true": results_json[filename]["structure_gt"],
                }
        ...
        for i, j in zip(samples, scores):
            if "span" in results_json[i]["structure_gt"]:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-n", "--njob", default=20, help="number of jobs in parallel")
    ...
    teds = TEDS(structure_only=args.structure, n_jobs=args.njob)
